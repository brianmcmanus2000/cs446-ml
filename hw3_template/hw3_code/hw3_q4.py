...
def split_data(X, y, train_size=0.7, val_size=0.15, test_size=0.15, random_state=42):
    ...
    # val and test are equal halves of the temp (because 0.15/0.30 = 0.5)
...
def check_early_stop(w, w_prev, tol):
    ...
    return np.max(np.abs(w - w_prev)) < tol
...
y_train_log = y_train_series.apply(np.log1p)
...
def smear_back_transform(y_train_log, train_pred_log, X_test_b, w):
    ...
    """
    y_log_np = np.asarray(y_train_log)          # handles Series/ndarray
    yhat_log_np = np.asarray(train_pred_log)    # ensure ndarray

    # safety: matching shapes
    if yhat_log_np.ndim > 1:
        yhat_log_np = yhat_log_np.ravel()

    residuals_log = y_log_np - yhat_log_np
    smear = float(np.mean(np.exp(residuals_log)))
